...
describe Noths::CheckoutSystem::Rules::DiscountRules::
  ...
  context "#apply" do
    context "basket object's price > total price limit" do
      it "applies percentage discount" do
        instance.apply(basket_object)
        expect(basket_object.total_checkout_price.to_s).to eq("66.78")
      end
    end
    ...
    context "basket object's price <= total price limit" do
      ...
      it "doesn't apply the discount" do
        instance.apply(basket_object)
        expect(basket_object.total_checkout_price.to_s).to eq("#{total_price}")
      end
    end
  end
end